			...
			else if (tp_dev.y[0] > BUTTON_FIRST_y + BUTTON_HEIGHT*2 && tp_dev.y[0] <= BUTTON_FIRST_y + BUTTON_HEIGHT * 3)
			{
				...
				if(tp_dev.x[0] > BUTTON_FIRST_x && tp_dev.x[0] <= BUTTON_FIRST_x + BUTTON_WIDTH)
				{
					...
					if(rgb_num == -1)
					{
						...
						RGB_DrawRectangle(1, 1, 3, 3, rgb_color[(color_index+ 1) % (sizeof(rgb_color) / sizeof(rgb_color[0]))]);
						RGB_DrawDotColor(2, 2, 1, rgb_color[(color_index+ 2) % (sizeof(rgb_color) / sizeof(rgb_color[0]))]);
					}
					else
					{
